...
### UI/UX 设计特色 (v1.2.0 新增)
- **设计理念**: 参考 Google Gemini 的简洁现代设计风格
- **布局结构**: 侧边栏 + 主聊天区域的经典布局
- **消息气泡**: 用户消息蓝色渐变右对齐，AI消息浅灰色左对齐带头像
- **输入区域**: Gemini风格圆角输入框，带聚焦动画和建议按钮
- **侧边栏**: 品牌标识、搜索功能、分组显示、交互动画
- **动画效果**: 页面加载、消息显示、输入交互的平滑动画
- **响应式设计**: 桌面、平板、移动端的完整适配

### 技术实现细节
- **消息状态管理**: pending、completed、failed 状态流转
- **动画系统**: MUI动画组件 + 自定义CSS过渡
- **主题系统**: 基于Gemini设计语言的自定义主题配置
- React + Material-UI 构建现代化界面
- WebSocket 实现实时通信
- 支持 Markdown 渲染和代码高亮  
...
- **Material-UI (MUI)**: Google Material Design 组件库
  - **动画组件**: Fade、Grow、Slide等过渡动画
  - **响应式工具**: useMediaQuery钩子实现断点检测
  - **主题系统**: 基于Gemini设计语言的自定义主题
- **React Router**: 前端路由管理
- **Axios**: HTTP 客户端
- **Socket.io-client**: 实时通信
- **React Markdown**: Markdown 渲染
- **Prism.js**: 代码语法高亮
- **CSS-in-JS**: 样式组件化，支持动态主题切换
